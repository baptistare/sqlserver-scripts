...
exec master..sp_whoisactive @output_column_list = '[dd hh:mm:ss.mss],[session_id],[blocking_session_id],[sql_text],[login_name],[host_name],[database_name],[program_name],[wait_info],[CPU],[reads],[writes],[physical_reads],[used_memory],[tempdb_allocations],tempdb_current],[status],[open_tran_count],[percent_complete],[start_time],[login_time],[request_id],[collection_time]'
...
--################################################################################
--gerar a tabela de histórico com as colunas de interesse
--################################################################################
...
EXEC sp_WhoIsActive 
    ...
    @return_schema = 1, @get_plans = 1, @get_outer_command = 1, @get_locks = 1, @get_additional_info = 1, @find_block_leaders = 1, @schema = @s OUTPUT
...
use DBA_Admin
go
CREATE TABLE Log_whoisactive ([dt_log] datetime NOT NULL, [dd hh:mm:ss.mss] varchar(8000) NULL,[session_id] smallint NOT NULL,[blocking_session_id] smallint NULL,[blocked_session_count] varchar(30) NULL,[locks] xml NULL,[additional_info] xml NULL,[sql_text] xml NULL,[sql_command] xml NULL,[query_plan] xml NULL,[login_name] nvarchar(128) NOT NULL,[host_name] nvarchar(128) NULL,[database_name] nvarchar(128) NULL,[program_name] nvarchar(128) NULL,[wait_info] nvarchar(4000) NULL,[CPU] varchar(30) NULL,[reads] varchar(30) NULL,[writes] varchar(30) NULL,[physical_reads] varchar(30) NULL,[used_memory] varchar(30) NULL,[tempdb_allocations] varchar(30) NULL,[tempdb_current] varchar(30) NULL,[status] varchar(30) NOT NULL,[open_tran_count] varchar(30) NULL,[percent_complete] varchar(30) NULL,[start_time] datetime NOT NULL,[login_time] datetime NULL,[request_id] int NULL);
--drop table tb_whoisactive

--executar e arquivo resultado
exec master..sp_whoisactive		@get_plans = 1, @get_outer_command = 1, @get_locks = 1, @get_additional_info = 1, @find_block_leaders = 1
							,	@output_column_list = '[collection_time], [dd hh:mm:ss.mss],[session_id],[blocking_session_id],[blocked_session_count],[locks],[additional_info],[sql_text],[sql_command],[query_plan],[login_name],[host_name],[database_name],[program_name],[wait_info],[CPU],[reads],[writes],[physical_reads],[used_memory],[tempdb_allocations],[tempdb_current],[status],[open_tran_count],[percent_complete],[start_time],[login_time],[request_id]'
							,	@destination_table = 'DBA.dbo.Log_whoisactive'


--
select * from tb_whoisactive
select count(*) from tb_whoisactive--0
